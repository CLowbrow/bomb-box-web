...
test("site uses repository-path-safe relative asset URLs", async () => {
  ...
  const editorHtml = await readFile(
    new URL("editor/index.html", outputRoot),
    "utf8",
  );
...
test("site renders the engine's expanded board state through Three.js", async () => {
  ...
  const threeView = await readFile(
    new URL("../site/game/three-view.js", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    html,
    /role="img" aria-label="Loading three-dimensional game board"/,
  );
...
test("expanded browser level preserves the engine hardening core", async () => {
  ...
  assert.deepEqual(
    builtLevel.entities.slice(0, contractLevel.entities.length),
    contractLevel.entities,
  );
  assert.deepEqual(
    builtLevel.fixtures.slice(0, 2),
    contractLevel.fixtures.slice(0, 2),
  );
  assert.deepEqual(
    builtLevel.cells.filter(
      ({ coordinate }) => coordinate.x <= 6 && coordinate.y <= 1,
    ),
...
test("minimal game surface includes recovery, JSON-backed levels, and console turn logging", async () => {
  ...
  assert.match(html, /id="fullscreen"[^>]*aria-pressed="false"/);
...
test("canned level folder contains valid editable JSON levels", async () => {
  const entries = (await readdir(new URL("levels/canned/", outputRoot))).filter(
  ...
  try {
    for (const entry of entries) {
      const level = JSON.parse(
        await readFile(new URL(`levels/canned/${entry}`, outputRoot), "utf8"),
      );
      assert.equal(
        engine.loadLevel(level).status,
        "loaded",
        `${entry} should load in the engine`,
      );
    }
  } finally {
    engine.destroy();
...
test("expanded route walks on a box, pushes a box, traverses the ramp, and wins", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      {
        coordinate: rampEntry.coordinate,
        bottomHalfSteps: rampEntry.bottomHalfSteps,
      },
    ...
    assert.deepEqual(
      {
        coordinate: rampExit.coordinate,
        bottomHalfSteps: rampExit.bottomHalfSteps,
      },
